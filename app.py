...
def update_checkout_date():
    """
    Automatically set checkout to one day after check-in.
    """
    st.session_state.check_out_date = (
        st.session_state.check_in_date + timedelta(days=1)
    )
    ...
    if check_out_date <= check_in_date:
        ...
        with st.status(
            ...
            try:
                ...
                st.session_state.result = None
                st.session_state.error = (
                    f"Unable to fetch room availability: {error}"
                )
...
if result:
    ...
    result_check_in = result.get("check_in", "")
    result_check_out = result.get("check_out", "")
    ...
    st.info(
        f"Check-in: {result_check_in} → "
        f"Check-out: {result_check_out}"
    )
    ...
    # Display the fetched result as plain text.
    st.code(
        availability_text,
        language=None,
    )
        ...
        st.html(
            ...
            <div class="empty-state">
                ...
                <div class="empty-state-description">
                    Select your stay dates above and click
